...
export const algorithms = [
  ...
  item('kd-tree','KD-Tree','Árboles','tree','Promedio O(log n)','Particiona puntos alternando ejes para búsquedas espaciales. Cada número de dos dígitos representa un punto XY.',`insertar(punto, profundidad):\n  eje ← profundidad mod k\n  comparar por eje\n  descender izquierda o derecha`,[55,25,75,12,38,61,89]),
  item('quadtree','QuadTree','Árboles','tree','Depende de distribución','Divide recursivamente un plano en cuatro cuadrantes.',`insertar(punto):\n  si capacidad disponible: guardar\n  si no: subdividir en 4\n  insertar en cuadrante correspondiente`,['-55,-45','48,-62','-30,52','62,44','15,-18','-72,70','78,8']),
  item('octree','Octree','Árboles','tree','Depende de distribución','Extiende el QuadTree a 3D dividiendo el espacio en ocho octantes.',`subdividir(cubo):\n  crear 8 octantes\n  distribuir puntos\n  repetir si se supera capacidad`,['-55,-45,-30','48,-62,35','-30,52,-48','62,44,55','15,-18,72','-72,70,10','78,8,-65','22,64,42']),
  ...
  item('bubble-sort','Bubble Sort','Ordenamientos','sort','Promedio y peor O(n²) · Mejor O(n)','Compara pares vecinos e intercambia los que están en orden incorrecto; cada pasada deja el mayor valor pendiente al final.',`bubbleSort(A):\n  para fin ← n-1 hasta 1\n    huboCambio ← falso\n    para i ← 0 hasta fin-1\n      si A[i] > A[i+1]: intercambiar\n    si no huboCambio: terminar`,[29,10,14,37,13,8,22]),
  item('selection-sort','Selection Sort','Ordenamientos','sort','Tiempo O(n²) · Espacio O(1)','Busca el menor elemento de la zona no ordenada y lo coloca al comienzo en cada pasada.',`selectionSort(A):\n  para i ← 0 hasta n-2\n    menor ← i\n    para j ← i+1 hasta n-1\n      si A[j] < A[menor]: menor ← j\n    intercambiar A[i] con A[menor]`,[31,7,25,12,40,3,18]),
  item('insertion-sort','Insertion Sort','Ordenamientos','sort','Promedio y peor O(n²) · Mejor O(n)','Construye una zona ordenada insertando cada nuevo valor en su posición correcta mediante desplazamientos.',`insertionSort(A):\n  para i ← 1 hasta n-1\n    clave ← A[i]\n    desplazar hacia la derecha los mayores que clave\n    insertar clave en el espacio libre`,[18,6,27,4,19,11,23]),
  item('merge-sort','Merge Sort','Ordenamientos','sort','Tiempo O(n log n) · Espacio O(n)','Divide recursivamente el arreglo en mitades y combina las partes ya ordenadas usando un arreglo auxiliar.',`mergeSort(A):\n  dividir A en izquierda y derecha\n  mergeSort(izquierda)\n  mergeSort(derecha)\n  mezclar ambas`,[38,12,27,5,19,44,8]),
  item('quick-sort','Quick Sort','Ordenamientos','sort','Promedio O(n log n) · Peor O(n²)','Particiona el rango alrededor de un pivote y luego ordena recursivamente los valores de cada lado.',`quickSort(A):\n  pivote ← elegir(A)\n  posición ← particionar alrededor del pivote\n  quickSort(lado izquierdo)\n  quickSort(lado derecho)`,[33,10,55,21,4,47,18]),
  item('shell-sort','Shell Sort','Ordenamientos','sort','Depende de los saltos · Usualmente entre O(n log n) y O(n²)','Mejora Insertion Sort comparando primero elementos alejados y reduciendo el salto hasta llegar a uno.',`shellSort(A):\n  salto ← n / 2\n  mientras salto > 0\n    aplicar inserción usando ese salto\n    salto ← salto / 2`,[42,17,8,33,5,26,11,19]),
  item('heap-sort','Heap Sort','Ordenamientos','sort','Tiempo O(n log n) · Espacio O(1)','Construye un max-heap y mueve repetidamente su raíz al final, restaurando el heap después de cada extracción.',`heapSort(A):\n  construir maxHeap\n  para fin ← n-1 hasta 1\n    intercambiar A[0] con A[fin]\n    heapify(A, 0, fin)`,[21,35,12,9,28,17,4,31]),
  item('counting-sort','Counting Sort','Ordenamientos','sort','Tiempo O(n + k) · Espacio O(k)','Cuenta cuántas veces aparece cada valor y reconstruye el arreglo en orden. Esta versión usa un desplazamiento para admitir negativos.',`countingSort(A):\n  hallar mínimo y máximo\n  crear conteo[max-min+1]\n  contar cada valor usando valor-min\n  reconstruir A desde conteo`,[4,-2,7,4,1,-2,5,1]),
  item('radix-sort','Radix Sort','Ordenamientos','sort','Tiempo O(d · (n + 10)) · Espacio O(n)','Ordena por unidades, decenas y demás dígitos con Counting Sort estable. Un desplazamiento permite incluir números negativos.',`radixSort(A):\n  desplazar claves para que sean no negativas\n  para exp ← 1, 10, 100...\n    ordenar establemente por el dígito actual`,[170,45,75,-90,802,24,2,66]),
  item('bogo-sort','Bogo Sort','Ordenamientos','sort','Promedio O(n · n!) · Sin cota práctica segura','Mezcla el arreglo al azar hasta que, por casualidad, queda ordenado. La demostración limita los intentos mostrados para evitar una ejecución interminable.',`bogoSort(A):\n  mientras A no esté ordenado\n    mezclar A al azar\n  retornar A`,[3,1,4,2]),
...
export const categoryLabels = {
  ...
  Ordenamientos: 'Comparación, distribución y orden',
