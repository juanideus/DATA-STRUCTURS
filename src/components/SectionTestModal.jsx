...
function QuestionVisual({ visual }) {
  ...
  if (linearTypes.has(type)) {
    drawing = <>
      ...
      {type === 'cache' && <text className="diagram-label" x="75" y="122">menos reciente → más reciente</text>}
    </>;
  } else if (type === 'stack') {
    ...
    drawing = <>
      ...
      {type === 'merkle' && <text className="diagram-label" x="132" y="8">hash raíz</text>}
      {type === 'heap' && <text className="diagram-label" x="124" y="8">máximo</text>}
    ...
    drawing = <>
      ...
      {type === 'sparse-matrix' && <text className="diagram-label" x="225" y="76">solo ≠ 0</text>}
    </>;
  } else if (type === 'hash' || type === 'bloom') {
    drawing = <>
      <rect x="28" y="52" width="65" height="42" rx="8"/><text x="60" y="77">clave</text><path d="M96 73 L132 73"/>
      ...
      <text className="diagram-label" x="120" y="118">{type === 'bloom' ? 'bits activados' : 'función hash → cubeta'}</text>
    </>;
  } else if (type === 'hanoi') {
    ...
    drawing = <><rect x="105" y="20" width="100" height="65" rx="8"/><text x="155" y="43">Clase</text><path d="M115 52 L195 52"/><text className="diagram-label" x="125" y="70">estado + métodos</text><path d="M155 87 L95 112 M155 87 L215 112"/><circle cx="90" cy="120" r="20"/><circle cx="220" cy="120" r="20"/><text className="diagram-label" x="72" y="148">objeto</text><text className="diagram-label" x="202" y="148">objeto</text></>;
  } else {
  ...
  return <figure className={`section-test-diagram diagram-${type}`} aria-label={`Diagrama conceptual: ${visual.caption}`}>
